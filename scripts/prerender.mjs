...
import { optimizeUnsplashImages } from "./lib/unsplash-optimize.mjs";

const __dirname = new URL(".", import.meta.url).pathname;
const workspaceRoot = path.resolve(__dirname, "..");

/**
 * LPS-943 — Run the Unsplash image optimiser over `html`. Returns the
 * mutated HTML on success, falls back to the input on any failure so a
 * parser/regex hiccup never blocks publish. Mirrors the fail-open pattern
 * of the surrounding prerender loop.
 */
function safeOptimizeImages(html, label) {
  try {
    return optimizeUnsplashImages(html);
  } catch (e) {
    console.warn(`[prerender] ⚠  image optimiser failed for ${label}: ${e.message}`);
    return html;
  }
}
...
try {
  ...
  // LPS-320 — optional; older SSR bundles won't have this export.
  renderBlogPost = typeof mod.renderBlogPost === "function" ? mod.renderBlogPost : null;
} catch (e) {
...
for (const page of pages) {
  ...
  try {
    ...
    const optimised = safeOptimizeImages(patched, `slug="${slug}"`);
    writeFileSync(htmlPath, optimised, "utf8");
    rendered++;
...
// ── LPS-320 — Pre-render blog post pages ─────────────────────────────────────
// Reads src/data/blog.json (written by the agent's blog_data_sync helper at
// Step 1.5 of handle_build_and_publish). For each post, locates the matching
// dist/<page-slug>/blog/<post-slug>/index.html (emitted by Rollup via the
// mpaHtmlGeneratorPlugin's per-post entries) and replaces its <div id="root">
// with the rendered <BlogArticle/> HTML.
//
// Fully non-fatal — same pattern as the manifest-page loop above.
if (renderBlogPost) {
  const blogJsonPath = path.join(workspaceRoot, "src", "data", "blog.json");
  let blogData = null;
  if (existsSync(blogJsonPath)) {
    try {
      blogData = JSON.parse(readFileSync(blogJsonPath, "utf8"));
    } catch (e) {
      console.log(`[prerender] Could not read src/data/blog.json: ${e.message}`);
    }
  }

  const pagesWithBlog = blogData?.pages_with_blog ?? [];
  const posts = blogData?.posts ?? [];

  if (posts.length > 0) {
    // Build lookup so we can resolve page_id → page meta for the article props.
    const pageMetaById = new Map(pagesWithBlog.map((p) => [p.page_id, p]));

    for (const post of posts) {
      const pageMeta = pageMetaById.get(post.page_id);
      if (!pageMeta) {
        console.warn(`[prerender] ⚠  Skipping post '${post.slug}' — page ${post.page_id} not in pages_with_blog`);
        skipped++;
        continue;
      }
      const pageSlug = pageMeta.page_slug ?? "";

      const postHtmlPath = pageSlug
        ? path.join(workspaceRoot, "dist", pageSlug, "blog", post.slug, "index.html")
        : path.join(workspaceRoot, "dist", "blog", post.slug, "index.html");

      if (!existsSync(postHtmlPath)) {
        console.warn(`[prerender] ⚠  HTML missing for post '${post.slug}': ${postHtmlPath}`);
        skipped++;
        continue;
      }
      ...
      let renderedHtml = "";
      try {
        renderedHtml = renderBlogPost(post.page_id, pageSlug, pageMeta.page_name ?? "", post.slug);
      } catch (e) {
        console.warn(`[prerender] ⚠  renderBlogPost threw for '${post.slug}': ${e.message}`);
        skipped++;
        continue;
      }

      if (!renderedHtml) {
        console.warn(`[prerender] ⚠  Empty render for post '${post.slug}'`);
        skipped++;
        continue;
      }

      try {
        const shell = readFileSync(postHtmlPath, "utf8");
        if (!shell.includes(ROOT_PLACEHOLDER)) {
          console.warn(`[prerender] ⚠  Root placeholder not found in ${postHtmlPath}`);
          skipped++;
          continue;
        }
        const patched = shell.replace(ROOT_PLACEHOLDER, `<div id="root">${renderedHtml}</div>`);
        const optimised = safeOptimizeImages(patched, `post="${post.slug}"`);
        writeFileSync(postHtmlPath, optimised, "utf8");
        rendered++;
        const routePath = pageSlug ? `/${pageSlug}/blog/${post.slug}` : `/blog/${post.slug}`;
        console.log(`[prerender] ✓ ${routePath}`);
      } catch (e) {
        console.warn(`[prerender] ⚠  Write failed for ${postHtmlPath}: ${e.message}`);
        skipped++;
      }
    }
  }
} else {
  console.log("[prerender] renderBlogPost export missing — older SSR bundle, blog posts not pre-rendered");
}
