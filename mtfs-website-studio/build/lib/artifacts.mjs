...
/**
 * Join fragments with newlines, KEEPING `''` as a deliberate blank line — the
 * artifacts this module writes are read by humans as well as crawlers, and the
 * blank line between a robots.txt group or an llms.txt section is structural.
 * Non-strings are dropped, so a conditional fragment can be `null`.
 */
function lines(parts) {
  return parts.filter((p) => typeof p === 'string').join('\n');
}

/** As `lines`, but `''` is dropped too — for places where a blank line would be wrong. */
 ...
 *       (b) exactly one side is a closed-set state word — "No", "Yes", "Free",
 *           "None", "All", "N/A", … — which is what pairs "No" with "Purchase
 *           Obligations" and "Yes" with "Immediate Savings".
 *
 * The prose side becomes the label and the value side the value, so the pair
 * reads "Cost to Join: $0" regardless of DOM order.
 *
 * !! WHY (b) IS A CLOSED SET AND NOT A LENGTH HEURISTIC. "Short side is the
 * value" is FALSE on this corpus. `/services/lab-solutions/staffing-solutions/`
 * ships `<div>Embryologists</div><div>Senior &amp; Junior positions</div>` —
 * title first, subtitle second — while `/services/lab-solutions/gpo-purchasing/`
 * ships `<div>No</div><div>Purchase Obligations</div>` — value first, label
 * second. Identical shapes, opposite semantics, so any length-based rule
 * inverts one of them ("Senior & Junior positions: Embryologists"). Reordering
 * is only allowed where the value is self-identifying. Everything the tests
 * cannot resolve is emitted as two plain lines: never merged, never reordered,
 * never dropped. This rule only ever REASSEMBLES.
 */
function pairStatTiles(blocks) {
  ...
  // (b) closed-set state-word test, only when the numeric test did not resolve it
  if (valueFirst === null) {
    const stateA = STATE_VALUES.has(a.text.toLowerCase());
    const stateB = STATE_VALUES.has(b.text.toLowerCase());
    if (stateA !== stateB) {
      const label = stateA ? b.text : a.text;
      if (label.length >= 4) valueFirst = stateA;
    }
  }
...
/**
 * The only non-numeric strings allowed to be treated as the VALUE half of a
 * tile. Closed on purpose — see the note in pairStatTiles: a length heuristic
 * inverts real pairs on this corpus.
 */
const STATE_VALUES = new Set([
  'yes', 'no', 'none', 'free', 'all', 'n/a', 'na', 'unlimited', 'included', 'tbd',
]);
